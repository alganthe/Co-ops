...
private _randomPos = [_AOPos, derp_PARAM_AOSize, "(1 + sea) * (1 + (waterDepth factor [0.1, 0.5]))"] call derp_fnc_randomPos;
...
if (_randomPos isEqualTo []) then {
    _randomPos = [_AOPos, derp_PARAM_AOSize, "(1 + meadow) * (1 - sea) * (1 - houses)"] call derp_fnc_randomPos;
    ...
    for "_derpSMgrp" from 1 to 2  step 1 do {
        private _infantryGroup = [([(position _cargoBox), 100, "(1 + meadow) * (1 - sea) * (1 - houses)"]), EAST, (configFile InfantryGroupsCFGPATH (selectRandom InfantryGroupList))] call BIS_fnc_spawnGroup;
    ...
    for "_derpSMgrp" from 1 to 3 step 1 do {
        private _scubaGroup = [([(position _cargoBox), 30, "(1 + sea) * (1 + (waterDepth factor [0.1, 0.5]))"] call derp_fnc_randomPos), EAST, (configFile CARGOSMScubaGroup)] call BIS_fnc_spawnGroup;

        deleteWaypoint [_scubaGroup, 0];
        for "_derpWp" from 1 to 3 step 1 do {
            private _waypoint = _scubaGroup addWaypoint [([_cargoBox, 150, "(1 + sea)"] call derp_fnc_randomPos), 10];
